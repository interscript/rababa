...
def extract_stack(stack, correct_reversed)
    ...
    if Arabic_constant::ALL_POSSIBLE_HARAQAT.include? full_haraqah
        ...
        #val = [Arabic_constant::ALL_POSSIBLE_HARAQAT[diacritic] for diacritic in full_haraqah]
        #val = val.join('|')
        raise ValueError.new("""The chart has the following haraqat which are not found in
                                 all possible haraqat: {val}""")
    end
