...
class TextEncoder:
    ...
    def __init__(
        ...
        self.d_target_symbols = dict(
            [(k, [TextEncoder.pad] + self.target_model[k]) for k in target_model.keys()]
        )

        self.input_symbol_to_id: Dict[str, int] = {
            s: i for i, s in enumerate(self.input_symbols)
        }
        self.d_input_id_to_symbol = dict(
            [
                (k, {s: i for i, s in enumerate(self.input_symbols)})
        ...
        self.d_target_symbol_to_id = dict(
            [
                (k, {s: i for i, s in enumerate(self.d_target_symbols[k])})
                for k in self.d_target_symbols.keys()
            ]
        )

        self.d_target_id_to_symbol = dict(
            [
                (k, {i: s for i, s in enumerate(self.d_target_symbols[k])})
                for k in self.d_target_symbols.keys()
            ]
        )
        ...
        self.d_target_pad_id = dict(
            [
                (k, self.d_target_symbol_to_id[k][self.pad])
                for k in self.d_target_symbols.keys()
            ]
        )
    ...
    def input_to_sequence(self, text: str) -> List[int]:
        ...
        sequence = [
            self.input_symbol_to_id[s]
            for s in text
            if s not in [self.pad] and self.input_symbol_to_id.get(s, False)
        ]
        if len(sequence) == 0:
    ...
    def target_to_sequence(self, text: str) -> List[int]:
        ...
        for k in self.d_target_symbol_to_id.keys():
            d_sequence[k] = [
                self.d_target_symbol_to_id[k].get(s, 0)
                for s in text
                if s not in [self.pad]
            ]
        return d_sequence
    ...
    def sequence_to_target(self, d_sequence):  # List[int]):
        ...
        for k in self.input_symbols.keys():
            d_target[k] = [
                self.d_target_id_to_symbol[symbol]
                for symbol in sequence
                if symbol in self.d_target_id_to_symbol[k]
                and symbol not in [self.target_pad_id]
            ]
        return d_target
