...
class Diacritizer:
    ...
    def get_data_from_file(self, path):
        ...
        loader_params = {
            "batch_size": self.config_manager.config["batch_size"],
            "shuffle": False,
            "num_workers": 0,
        }

        dataset = DiacritizationDataset(self.config_manager, path)
        data_iterator = DataLoader(dataset.data, collate_fn=collate_fn, **loader_params)
    ...
    def diacritize_file(self, path: str, path_out: str):
        ...
        def postprocess_data(raw_data):
            return raw_data.cpu().numpy() if self.device == "cuda" else raw_data

        dia_total = nakdimon_dataset.merge_unconditional(
            raw_data.text,
            postprocess_data(raw_data.normalized),
            postprocess_data(dia_data.niqqud),
            postprocess_data(dia_data.dagesh),
    ...
    def diacritize_data_iterator(self, data_iterator, criterion=None):
