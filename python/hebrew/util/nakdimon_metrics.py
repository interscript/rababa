...
def metric_cha(actual: str, expected: str, *args, **kwargs) -> float:
    ...
    return mean_equal(
        (x, y) for x, y in zip(actual_hebrew, expected_hebrew) if hebrew.can_any(x.letter)
    )
...
def metric_dec(actual: str, expected: str, *args, **kwargs) -> float:
    ...
    return mean_equal(
        (
            (x.niqqud, y.niqqud)
            for x, y in zip(actual_hebrew, expected_hebrew)
...
def metric_wor(actual: str, expected: str, *args, **kwargs) -> float:
    ...
    #         print()
    return mean_equal((x, y) for x, y in zip(actual_tokens, expected_tokens) if is_hebrew(x))
...
def get_items(
    actual: str, expected: str, vocalize=False
) -> tuple[list[hebrew.HebrewChar], list[hebrew.HebrewChar]]:
    ...
    diff = get_diff(
        repr("".join(c.letter for c in actual_hebrew)),
...
def all_diffs_for_files(expected_filename, system1, system2):
    ...
    actual_sentences1 = split_to_sentences(
        clean_read(expected_filename.replace("expected", system1))
    )
    actual_sentences2 = split_to_sentences(
        clean_read(expected_filename.replace("expected", system2))
    )
    assert len(expected_sentences) == len(actual_sentences1) == len(actual_sentences2)

    triples = [
        (e, a1, a2)
        for (e, a1, a2) in zip(expected_sentences, actual_sentences1, actual_sentences2)
...
def collect_failed_words_for_files(system):
    for folder in basepath.iterdir():
        for file in folder.iterdir():
            ...
            expected_sentences = split_to_sentences(clean_read(expected_filename))
            actual_sentences = split_to_sentences(
                clean_read(expected_filename.replace("expected", system))
            )
            ...
            expected_tokens = [
                token for sentence in expected_sentences for token in sentence.split()
            ]
