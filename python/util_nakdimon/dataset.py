...
class CharacterTable:
    ...
    def to_str(self, ids):
        return [
            [self.indices_char[c] for c in cs] for cs in ids
        ]
...
def read_corpora(base_paths):
    return tuple([(filename, list(hebrew.iterate_file(filename)))
...
def load_data(corpora, validation_rate: float, maxlen: int,
    ...
    if validation_rate > 0:
        ...
        while total_size < validation_size:
            if abs(total_size - validation_size) < \
                    abs(total_size + len(corpus[-1]) - validation_size):
                break
