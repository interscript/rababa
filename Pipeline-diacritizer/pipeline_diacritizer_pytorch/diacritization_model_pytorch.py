...
from pipeline_diacritizer_pytorch.pytorch_nnet import PipelineModel, train_model
...
class DiacritizationModel:
    ...
    def __init__(self, save_dir='.', use_rules=True, use_trigrams=True, use_bigrams=True, use_unigrams=True,
        ...
        self.model = PipelineModel(len(CHAR2INDEX), self.TIME_STEPS)
        self.model.float()
    ...
    def train(self, train_sentences, val_sentences, epochs, early_stop_iter):
        ...
        train_model(self.model, train_ins, train_outs, nb_epochs=15)
    ...
    def diacritize_processed(self, u_p_text):
        ...
        # prepare data
        in_data, (target_shadda, target_harakat) = self.model.text_indices_2_data(text_indices, diacritics_indices, idx)
        # predict through the network
        predict_shadda, predict_harakat = self.model.forward(torch.tensor(in_data).float())
        shaddat = [NAME2DIACRITIC['Shadda'] if x >= 0.5 else '' for x in predict_shadda]
        harakat = [self.index_to_diacritic(np.argmax(x)) for x in predict_harakat]
